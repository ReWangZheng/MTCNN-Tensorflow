...
import numpy as np
class TrainDateSet:
    def __init__(self,sess,Net='Onet'):
        if Net=='onet':
        ...
        self.sess = sess
    def init(self):
        self.face_data = self.init_dataset(self.face_d)
        self.bound_data = self.init_dataset(self.bound_d)
        self.landmark_data = self.init_dataset(self.landmark_d)
    def get_data(self):
        x_face,y_face = self.sess.run(self.face_data)
        x_bound,y_bound = self.sess.run(self.bound_data)
        x_l,y_l = self.sess.run(self.landmark_data)
...
if __name__ == '__main__':
    sess = tf.Session()
    data = TrainDateSet(sess,Net='pnet')
    data.init()
    import matplotlib.pyplot  as plt
    import cv2
    for i in range(0,100):
        X, YC, YR, YL, label = data.get_data()
